...
CREATE TABLE if not exists file_obj_t (
    ...
    time_origin varchar(2000) not null,
    label varchar(2000),
...
CREATE INDEX if not exists idx_file_obj_t_1 ON file_obj_t (task_id);
CREATE INDEX if not exists idx_file_obj_t_2 ON file_obj_t (file_month);
CREATE INDEX if not exists idx_file_obj_t_3 ON file_obj_t (label);
